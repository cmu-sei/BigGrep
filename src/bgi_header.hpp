...
public:
    ...
    uint8_t hint_type; // added in format 2.1:
                       // 0 = "last" byte trimmed, 1 = "last" nybble trimmed,
                       // 2 = nothing trimmed/whole ngram used
    ...
    // "v3" == 2.2, adds compression to metadata section
    ...
    header_size()
    {
        // probably should calculate via adding sizeof()
        //return 28;
    ...
    inline uint32_t ngram_to_hint(uint32_t ngram)
    {
    ...
    dump()
    {
        ...
        oss << "  fileid_map_offset == 0x" << std::hex << fileid_map_offset
            << std::dec << std::endl;
        return oss.str();
    }
};

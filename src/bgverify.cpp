...
main(int argc, char** argv) {
  ...
  int ch;
  int option_index;
  while ((ch = getopt_long(argc,argv,"oVDh",long_options,&option_index)) != -1)
  {
    switch (ch)
    {
      case 'o':
  ...
  for (std::list< std::string >::iterator iter(files.begin()); iter != files.end(); ++iter) {
    ...
    // future (try and do searches simultaneously so only one pass through the
    // file front to back, allow for arbitrary combinations of AND/OR/NOT for
    // terms, etc), but for now lets just search for each of them individually
    ...
    for (int i(0); i < patterns.size(); ++i)
    {
      ...
      else
      {
        ...
        // and if we had started getting results, we no longer care due to
    ...
    if (! results.empty())
    {
      ...
      while (results.size() != 0)
      {
        if (showall)
        {
          ...
          {
            std::cout << (*iter) << ": " << std::setw(8) << std::setfill('0') << std::hex << i 
                      << std::dec << std::endl;
            ++num_matches;
          }
         }
  ...
  LINFO << num_matches << " matches across " << num_file_matches << " files (out of " << files.size() << ")" << std::endl;

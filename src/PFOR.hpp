...
public:
  ...
  {
    last_errorcode = 0;
    // use iterator math to verify size of the block (pad before calling if you need to)
    ...
    // okay, need to figure out our min b that gives <= maxexceptions values,
    ...
    for (unsigned i(0);i<_blocksize;++i)
    {
      ...
      // whoops, actually, we always need to round up, not ceil, because 32
      // (2^5) actually needs 6 bits to store it:
      minbits[i] = 1+ilog2< T >(*(vstart+i));
      // do we need to convert 0 to 1 here?  I think so, because the values 0
      // & 1 need to be stored in a single bit (and we'll be using b == 0 as a
      // special flag value later)
      //if (minbits[i] == 0)
      //{
      //  minbits[i] = 1;
      //}
      // since rounding up now, don't think minbits[i] can ever be 0, so don't check
    ...
    uint8_t b(0xFF);
    for (unsigned int i(1);i<_maxb;++i) // we know there are no values in bitcounts[0]
    {
    ...
    if (b == 0xFF) // couldn't meet requirements for b < 16 & maxexceptions
    {
      last_b = b;
      last_errorcode = 2;
    ...
    // okay, now know what our b is, so we can encode (will convert to vector later):
    uint8_t *outbuf = new uint8_t[sizeof(T)+b*_blocksize/8]; // pad out sizeof(T) to prevent over reads/writes
  ...
  // helper funcs to convert to/from delta lists...you can use the startval if
  // you don't want the first value in the vector to be the reference value
  static void convert_to_deltas(typename std::vector< T > &vec, T startval = 0)
  {
    for (unsigned i(vec.size()-1);i>0;--i)
    {
      vec[i] = vec[i] - vec[i-1];
    }
    // left out check for startval > 0, figuring check/jmp might be more expensive than sub of 0
    vec[0] = vec[0] - startval;
  }

  static void convert_from_deltas(typename std::vector< T > &vec, T startval = 0)
  {
    // left out check for startval > 0, figuring check/jmp might be more expensive than add of 0
    vec[0] = vec[0] + startval;
